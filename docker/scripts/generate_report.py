...
def parse_node_uart(filepath):
    events, checks = [], {"node_boot": False, "node_sensor": False, "node_lora": False}
...
def parse_master_uart(filepath):
    events, checks = [], {"master_boot": False, "master_poll": False, "master_humidity": False}
    humidity_readings = []
    if not os.path.exists(filepath):
    ...
    for line in raw.strip().splitlines():
        ...
        if "[MASTER] Poll #" in line:
            ...
            if hum_match:
                ...
                humidity_readings.append({"poll": poll_num, "humidity": float(hum_match.group(1))})
...
def generate_humidity_chart(humidity_readings, output_path):
    ...
    ax.plot(polls, humidities, color="#00d4ff", linewidth=2, marker="o",
            markersize=5, markerfacecolor="#00d4ff", markeredgecolor="#0f0f23")
    ...
    ax.set_title("Master Humidity Readings Over Time", color="#00d4ff", fontsize=11, pad=8, loc="left")
...
def build_report_data(node_checks, master_checks, node_events, master_events,
                      humidity_readings, node_raw, master_raw, chart_b64):
    check_list = [
        ...
        {"name": "Master humidity reading", "passed": master_checks["master_humidity"]},
    ]
    return {
...
def generate_html(report_data, template_path, output_path):
    ...
    injected = template.replace("// __REPORT_DATA_PLACEHOLDER__",
...
if __name__ == "__main__":
    ...
    master_events, master_checks, humidity_readings, master_raw = parse_master_uart(master_uart)
    print(f"[report] Master: {len(master_events)} events, {len(humidity_readings)} humidity readings")
    ...
    report_data = build_report_data(node_checks, master_checks, node_events, master_events,
                                    humidity_readings, node_raw, master_raw, chart_b64)
